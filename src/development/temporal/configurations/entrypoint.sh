...
# The auto-setup image has no support for Docker's `_FILE`-suffixed secret
# convention, so read the mounted secret files ourselves and hand the plain
# values to the image's own entrypoint.
